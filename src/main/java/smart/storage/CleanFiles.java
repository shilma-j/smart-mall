...
public class CleanFiles {
    ...
    public CleanFiles() {
        // 商品表引用到的静态文件
        AppConfig.getJdbcTemplate().queryForList("select des,imgs,spec from t_goods").forEach(row -> {
        ...
        // 订单商品表
        AppConfig.getJdbcTemplate().queryForList("select img from t_order_goods").forEach(row -> {
            files.add(getPath((String) row.get("img")));
        });
        // 商品规格
        AppConfig.getJdbcTemplate().queryForList("select list from t_spec").forEach(row -> {
            getUrlFromJson((String) row.get("list")).forEach(url -> files.add(getPath(url)));
        });
        // 首页幻灯片
        String json = AppConfig.getJdbcTemplate().queryForObject(
                "select value from t_system where entity='sys' and attribute='carousel'", String.class);
